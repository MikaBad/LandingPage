...
const Caroussel = () => {
  ...
  const ItemRestaurant2 = ({ item }: { item: any }) => {
    return (
      <div
        className={`${styles.productitem} restaurant-cards2`}
  ...
  const allRestaurantsCards = listPartenaire.map((item) => (
    <ItemRestaurant item={item} />
  ));
  const allRestaurantsCards2 = listPartenaire.map((item) => (
    <ItemRestaurant2 item={item} />
  ));
  ...
  const handleNavigation = (direction: any) => {
    ...
    document
      .querySelectorAll(".restaurant-cards")
      .forEach(
        (card: any) => (card.style.transform = `translate(${transformValue}%)`)
      );
    document
      .querySelectorAll(".restaurant-cards2")
      .forEach(
        (card: any) =>
          (card.style.transform = `translate(${transformValue - 50}%)`)
      );
